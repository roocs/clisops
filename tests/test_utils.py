...
def test_map_params_time():
    args = utils.map_params(time=("1999-01-01", "2100-12"), area=(0, -90, 360, 90))
    assert args["start_date"] == "1999"
    assert args["end_date"] == "2100"
...
def test_map_params_invalid_time():
    with pytest.raises(InvalidParameterValue):
        utils.map_params(
            time=("1999-01-01T00:00:00", "maybe tomorrow"), area=(0, -90, 360, 90)
        )
    with pytest.raises(InvalidParameterValue):
        utils.map_params(time=("", "2100"), area=(0, -90, 360, 90))


def test_map_params_area():
    args = utils.map_params(area=(0, 10, 50, 60),)
...
def test_map_params_invalid_area():
    with pytest.raises(InvalidParameterValue):
        utils.map_params(area=(0, 10, 50),)
    with pytest.raises(InvalidParameterValue):
        utils.map_params(area=("zero", 10, 50, 60),)
